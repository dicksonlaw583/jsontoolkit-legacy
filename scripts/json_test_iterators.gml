...
got = "";
for (var i = json_iterate(fixture, Array("bar"), ds_type_list); json_has_next(i); json_next(i)) {
    got += i[JSONITER.VALUE];
}
assert_equal(got, "abc", "12.2.4.1 (alt): Live list scenario failed!");
got = "";
for (var i = json_iterate(fixture, Array("baz"), ds_type_map); json_has_next(i); json_next(i)) {
    got += i[JSONITER.KEY]+":"+i[JSONITER.VALUE];
}
...
got = "";
for (var i = json_iterate(fixture, Array(-2), ds_type_map); json_has_next(i); json_next(i)) {
    got += i[JSONITER.KEY]+":"+i[JSONITER.VALUE];
}
